...
class ForgotPasswordModel {
    ...
    public function forgotPassword($data) {
        ...
        try {
            ...
            // 3) check rate limits (throws Exception 429 when exceeded)
            $this->isRateLimited($user["id"] ?? null, $ip);
            // 4) log the attempt (even if user is null, we log ip)
            $this->logPasswordResetRequest($user["id"] ?? null, $ip);
            ...
            $deleteOldTokenStmt->execute(["user_id" => $this->userId]);
            $isDeleted = $deleteOldTokenStmt->rowCount();
            
            $this->rawToken = bin2hex(random_bytes(32));
            $tokenHash = hash('sha256', $this->rawToken);
            //$expiresAt = (new DateTime('+1 hour'))->format('Y-m-d H:i:s');
            $expiresAt = (new DateTime('now', new DateTimeZone('UTC')))->add(new DateInterval('PT1H'))->format('Y-m-d H:i:s');
            ...
            if($insertNewTokenStmt->rowCount() === 0) {
                throw new Exception('Unos novog tokena nije uspeo', 422);
            }

            $this->link = "http://localhost:3000/reset-password?token={$this->rawToken}&user_id={$this->userId}";
            $userEmailData = [
            ...
            $isRecoveryPasswordEmailSent = $response['success'] ?? true;
            
            DatabaseModel::$pdo->commit();
            if(!$isRecoveryPasswordEmailSent) return [
                'success' => false,
            ...
            $this->logEmailError('error', $e->getMessage(), 'forgot_password_flow', $this->userId ?? null, $data['email'] ?? null);
            (isset(DatabaseModel::$pdo) && DatabaseModel::$pdo->inTransaction()) && DatabaseModel::$pdo->rollBack();
            throw $e;
        }
    }
 ...
 */
    ...
    private function sendRecoveryPasswordEmail($userEmailData) {
        ...
        try {
           ...
           // throw new Exception ('Pogrešan format: ' . $msg, 500);
    ...
    private function logEmailError($level, $message, $context = null, $userId = null, $email = null) {
        $saved = false;
